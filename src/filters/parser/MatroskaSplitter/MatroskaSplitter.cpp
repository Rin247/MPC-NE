...
const AMOVIESETUP_FILTER sudFilter[] = {
	{&__uuidof(CMatroskaSplitterFilter), MatroskaSplitterName, MERIT_NORMAL, std::size(sudpPins), sudpPins, CLSID_LegacyAmFilterCategory},
		...
		for (const auto& pTE : pT->TrackEntries) {
			...
			if (pTE->TrackType == TrackEntry::TypeVideo) {
				...
				if (CodecID == "V_MS/VFW/FOURCC") {
					...
					if (pTE->CodecPrivate.size() >= 23) {
						MPEG2VIDEOINFO* pm2vi = (MPEG2VIDEOINFO*)mt.pbFormat;
						const BYTE* extradata = pTE->CodecPrivate.data();
						...
						if (HEVCParser::ParseHEVCDecoderConfigurationRecord(extradata, size, params, false)) {
							...
							if (pTE->CodecPrivate.size() == 23) {
								...
								if (ReadFirtsBlock(pData, pTE.get())) {
									...
									if (HEVCParser::ReconstructHEVCDecoderConfigurationRecord(pData.data(), pData.size(), params.nal_length_size,
																							  extradata, size,
																							  new_record)) {
										CreateMPEG2VISimple(&mt, &pbmi, 0, aspect, new_record.data(), new_record.size(),
			...
			// Audio
				...
				if (CodecID == "A_MPEG/L3") {
					...
					mts.push_back(mt);
				} else if (StartsWith(CodecID, "A_AC3")) {
					...
					wfe->nAvgBytesPerSec = 0;
					mts.push_back(mt);
				} else if (StartsWith(CodecID, "A_AAC/")) {
					mt.subtype = FOURCCMap(wfe->wFormatTag = WAVE_FORMAT_RAW_AAC1);
				...
				} else if (StartsWith(CodecID, "A_REAL/") && CodecID.GetLength() >= 11) {
					...
					memcpy(wfe + 1, pTE->CodecPrivate.data(), pTE->CodecPrivate.size());
					mts.push_back(mt);
				} else if (StartsWith(CodecID, "A_OPUS")) {
			...
			// Subtitle
				...
				if (CodecID == "S_TEXT/ASCII") {
					...
					strncpy_s(psi->IsoLang, pTE->Language, std::size(psi->IsoLang) - 1);
					...
					wcsncpy_s(psi->TrackName, subtitle_Name, std::size(psi->TrackName) - 1);
					...
					mt.subtype =
						...
						CodecID == "D_WEBVTT/SUBTITLES" || CodecID == "S_TEXT/WEBVTT" ? MEDIASUBTYPE_WEBVTT :
