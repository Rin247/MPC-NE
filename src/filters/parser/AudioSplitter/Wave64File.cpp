...
const uint8_t w64_guid_id3 [16] = {'i', 'd', '3', ' ', 0xF3, 0xAC, 0xD3, 0x11, 0x8C, 0xD1, 0x00, 0xC0, 0x4F, 0x8E, 0xDB, 0x8A};
...
HRESULT CWave64File::Open(CBaseSplitterFile* pFile)
{
	...
	while (m_pFile->ByteRead(Chunk64.data, sizeof(Chunk64)) == S_OK && m_pFile->GetPos() < end) {
		...
		else if (memcmp(Chunk64.guid, w64_guid_id3, 16) == 0) {
			ReadID3Tag(Chunk64.size);
		}
#ifdef _DEBUG
